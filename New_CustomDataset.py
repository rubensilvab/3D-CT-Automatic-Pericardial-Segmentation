...
# combined_df = pd.concat([csv_file_abd, csv_file_cfat, csv_file_osic], ignore_index=True)

# df=combined_df.loc[combined_df['Fold'].isin(folds_train)]
# df.loc[:,'Patient']=df['Patient'].astype(str) 
# #df=csv_file_osic.loc[csv_file_osic['Fold'].isin(folds_train)]

# # # # transform = transforms.Compose([
# # # #     transforms.Resize((256, 256))
# # # #     ])

# dataset = PatientCustomDatasetCSVdcm(df, transform=transform_aug)

# indices,N_slices,SliPind=ExtractSliceInd(df)
    
# sampler=SubsetSampler(indices,N_slices,SliPind,batch_size=1)    

# # # for item in sampler:
# # #     print(item)
# # #     break

# data_loader_n = DataLoader(dataset, batch_sampler=sampler)

# # # # # batch_size = 4
# # # # # data_loader_o = DataLoader(dataset_old, batch_size=batch_size, shuffle=False)


# # # # Display image and label.
# # # #train_features, train_labels= next(iter(data_loader_n))
# # # # train_features, train_labels= next(iter(data_loader_o))
# # # #print(f"Feature batch shape: {train_features.size()}")
# # # #print(f"Labels batch shape: {train_labels.size()}")

# for i,(x,y) in enumerate(data_loader_n,0):
#     x,y=RandomCrop(x,y)
#     train_features, train_labels=x[0],y[0]
